...
- `Ctrl + 反引号键`（或 `F4`）打开方案菜单；“常规 / 聊天 / 写作 / 代码”是互斥场景，切换一个就会退出另一个。
- `rq` 日期、`sj` 时间、`xq` 星期、`dt` 固定 UTC+8 的 RFC 3339 日期时间、`ts` 时间戳、`nl` 农历、`uuid` 随机 UUID、`z` 重复上次上屏内容。
...
- 更新安装时会保留用户目录中已有的 `custom_phrase.txt`，不会再用仓库公开模板覆盖私人短语。
...
首次安装会复制公开的 `custom_phrase.txt` 模板；之后你在用户目录中加入的私密条目，以及本地生成的 `context_boost.tsv`、`context_boost.journal.tsv`、`predict.db` 等用户习惯记录，均属于你的**绝对私密资产**。本仓库不会收集或要求你上传这些数据；更新安装会保留已有私人短语。
